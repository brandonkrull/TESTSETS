...
Usage:
(1) Make sure the .csv file containing the reference results is in the ./ref
    directory.
...
# FC-CCSD(T)/CBS, energies in kJ/mol
# Haunschild and Klopper TCA 131, 1112 (2012)
mol_data = pd.read_csv('../ref/ref.csv')
mol_data['Ea CCSD(T)'] = (mol_data['Total'])
...
for fun in funs:
    ...
    for i, mol in enumerate(mols):
        ...
        ae_mols[i] = (sum_en_atoms - en_mol)* KJ_MOL_IN_AU
    # convert from Hartree to kJ/mol
    mol_data[fun] = ae_mols 
    mol_data[fun + ' err.'] = (mol_data[fun] - mol_data['Ea CCSD(T)'])*KJ_MOL_IN_KCAL_MOL
